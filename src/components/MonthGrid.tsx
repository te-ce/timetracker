...
export function MonthGrid({
  ...
  const { save: saveMutation, remove: deleteMutation } =
    useTimeEntryMutations(timeEntryRepository);
    ...
    mutationFn: async (row: MonthGridRow) => {
      ...
      if (autoCategory && autoHours > 0) {
        const existing = entries.find(
          (e) => e.date === row.date && e.category === autoCategory,
        );
  ...
  function handleBlur(row: MonthGridRow, category: string) {
    ...
    const existing = entries.find(
      (e) => e.date === row.date && e.category === category,
    );
  ...
  function getRowStatus(row: MonthGridRow): DayStatus {
    ...
    return getDayStatus({
      ...
      isEntriesBalanced:
        row.workedHours > 0 && Math.abs(row.workedHours - manualTotal) < 0.01,
  ...
  function cycleLocation(date: string) {
    const effective: WorkLocation =
      workLocations.get(date) ?? defaultWorkLocation ?? 'Remote';
  ...
  const sprintGroups = computeSprintGroups(
    rows,
    sprintStartDate,
    sprintLengthDays,
  );
      ...
      <div className="overflow-x-auto max-h-[75vh] overflow-y-auto relative">
        <table className="w-full text-sm border-collapse" role="table">
          <thead className="sticky top-0 z-20 bg-white shadow-sm">
            <tr>
              <th className="sticky left-0 z-30 bg-white px-2 py-1.5 text-left w-12 border-b">
                Day
              </th>
              <th
                className="sticky left-12 z-30 bg-white px-1 py-1.5 w-5 border-b"
                title="Status"
              ></th>
              <th
              ...
              <th className="px-1 py-1.5 text-center w-10 border-b text-xs border-l border-gray-200">
                📍
              </th>
              ...
              {allCategories.map((cat, catIdx) => (
                ...
                >
                  ...
                  <span
                    aria-hidden="true"
                    className="flex justify-center items-center h-[13px] mt-0.5"
                  >
                    {cat === autoCategory ? (
                      <span className="text-[9px] text-indigo-400 font-medium tracking-wide leading-none">
                        auto
                      </span>
                    ) : onAutoCategoryChange ? (
                      <button
          ...
          <tbody>
            {sprintGroups.map((group) => {
              const sprintWorked = group.rows.reduce(
                (s, r) => s + r.workedHours,
                0,
              );
              const groupRows = group.rows.map((row) => {
                ...
                const bgPair = isToday
                  ? TODAY_ROW_BG
                  : STATUS_ROW_BG[displayStatus];
                const rowBg = bgPair[globalRowIdx % 2];
                const loc: WorkLocation =
                  workLocations.get(row.date) ??
                  defaultWorkLocation ??
                  'Remote';
                const dayLabel = new Date(row.date)
                ...
                return (
                  ...
                  >
                    <td
                      className={`sticky left-0 z-10 px-2 py-1 font-mono text-xs ${rowBg}`}
                      title={row.date}
                    >
                      {row.date.slice(8)}
                    ...
                    <td className={`sticky left-12 z-10 px-1 py-1 ${rowBg}`}>
                      ...
                      >
                        <span
                          className={`inline-block h-2 w-2 rounded-full ${STATUS_DOT[displayStatus]}`}
                        />
                      </button>
                    </td>
                    <WorkedHoursCell
                    ...
                    {allCategories.map((cat) => {
                      ...
                      const hasAutoHours =
                        isAutoTarget && row.autoCategoryHours > 0;
                      const isDayConfirmed = confirmedDays.has(row.date);
                      return (
                        <td
                          key={cat}
                          className="px-0.5 py-0.5 w-16 min-w-[4rem] max-w-[4rem]"
                        >
                          {isDayConfirmed ||
                          (isAutoTarget &&
                            !row.entries[cat] &&
                            ...
                            >
                              {(() => {
                                ...
                                const auto = isAutoTarget
                                  ? row.autoCategoryHours
                                  : 0;
                ...
                <Fragment key={group.label}>
                  {group.label && (
                    <tr className="bg-indigo-50/60">
                      <td
                        colSpan={colCount}
                        className="px-2 py-1 text-xs font-semibold text-indigo-700 border-b"
                      >
                        {group.label}
                      </td>
                    </tr>
                    ...
                    <tr className="bg-indigo-50/40 border-t">
                      ...
                      {allCategories.map((cat) => {
                        const catTotal = group.rows.reduce((sum, row) => {
                          const manual = row.entries[cat] ?? 0;
                          const autoHours =
                            cat === autoCategory ? row.autoCategoryHours : 0;
          ...
          <tfoot className="sticky bottom-0 z-20 bg-white shadow-[0_-1px_3px_rgba(0,0,0,0.1)]">
            <tr className="border-t font-semibold">
              ...
              <td
                className="sticky left-[4.25rem] z-30 bg-white px-2 py-1 text-right"
              ...
              {allCategories.map((cat) => {
                ...
                return (
                  <td
                    key={cat}
                    className="px-1 py-1 text-right text-xs w-16 min-w-[4rem] max-w-[4rem]"
                  >
                    {catTotal > 0 ? catTotal.toFixed(2) : ''}
                  </td>
                );
