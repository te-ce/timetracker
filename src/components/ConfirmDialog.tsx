...
export function ConfirmDialog({ title, message, confirmLabel = 'Confirm', onConfirm, onCancel, danger = false }: Props) {
  ...
  return createPortal(
    <>
      <button className="fixed inset-0 z-[100] bg-black/30 cursor-default" aria-label="Close dialog" onClick={onCancel} />
      <div
        ref={ref}
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex flex-col gap-1">
          <h2 id="confirm-dialog-title" className="text-base font-semibold dark:text-gray-100">{title}</h2>
