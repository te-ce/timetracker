...
export const VARIANTS = [
  ...
  { key: 'B', name: 'Sidebar Dashboard' },
  { key: 'C', name: 'Continuous Timeline' },
] as const
...
export function PrototypeSwitcher({ current }: { current: VariantKey }) {
  ...
  const currentDef = VARIANTS[index] ?? VARIANTS[0]

  const go = useCallback(
    (delta: number) => {
      const nextIndex = (index + delta + VARIANTS.length) % VARIANTS.length
      const nextKey = (VARIANTS[nextIndex] ?? VARIANTS[0]).key
      void navigate({ to: '/prototype', search: { variant: nextKey } })
    },
    [index, navigate],
  )
