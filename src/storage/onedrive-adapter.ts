...
export class OneDriveStorageAdapter implements StorageAdapter {
  ...
  async get<T>(key: string): Promise<T | null> {
    ...
    const res = await fetch(`https://graph.microsoft.com/v1.0/me/drive/special/approot:/${key}:/content`, {
  ...
  async delete(key: string): Promise<void> {
    const token = await this.tokenProvider()
    const res = await fetch(`https://graph.microsoft.com/v1.0/me/drive/special/approot:/${key}:`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    })
