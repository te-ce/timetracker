...
describe('AutoCategoryRow', () => {
  it('shows calculated auto hours for the configured category', () => {
    render(<AutoCategoryRow autoCategory="_COREMEDIA" workedHours={8} manualTotal={5} />, { wrapper })
  ...
  it('shows 0h and overbooking warning when manual exceeds worked', () => {
    render(<AutoCategoryRow autoCategory="_SUPPORT" workedHours={6} manualTotal={9} />, { wrapper })
    expect(screen.getByText('0h')).toBeInTheDocument()
    expect(screen.getByText(/overbooking/i)).toBeInTheDocument()
  })

  it('renders nothing when autoCategory is null', () => {
    const { container } = render(<AutoCategoryRow autoCategory={null} workedHours={8} manualTotal={3} />, { wrapper })
    expect(container).toBeEmptyDOMElement()
  })

  it('shows exactly 0h when manual equals worked', () => {
    render(<AutoCategoryRow autoCategory="_INFRA" workedHours={8} manualTotal={8} />, { wrapper })
    expect(screen.getByText('0h')).toBeInTheDocument()
