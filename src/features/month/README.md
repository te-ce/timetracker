...
5. A day balance is only shown once it is knowable — a future date, or a day with nothing tracked, shows none rather than a full negative.
6. Week and month targets count only days with tracked hours, the same rule the overtime math uses; untracked past workdays surface as gaps in the attention strip and the "N days untracked / Xh missing" chip instead.
